...
void* gpu_buffer::lock() {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_buffer::lock");
...
void gpu_buffer::unlock() {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_buffer::unlock");
    std::lock_guard<std::mutex> locker(_mutex);
    _lock_count--;
    if (0 == _lock_count) {
...
void gpu_buffer::zero_buffer() {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_buffer::zero_buffer");
...
void gpu_buffer::fill(unsigned char pattern, event_impl::ptr ev) {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_buffer::fill");
...
void gpu_image2d::zero_image() {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_image2d::zero_image");
...
void* gpu_image2d::lock() {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_image2d::lock");
    std::lock_guard<std::mutex> locker(_mutex);
    if (0 == _lock_count) {
...
gpu_usm::gpu_usm(const refcounted_obj_ptr<engine_impl>& engine, const layout& layout, uint32_t net_id, allocation_type type, bool reset)
    ...
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_usm::gpu_usm");
...
void* gpu_usm::lock() {
    ...
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_usm::lock");
    std::lock_guard<std::mutex> locker(_mutex);
    if (0 == _lock_count) {
...
void gpu_usm::fill(unsigned char pattern, event_impl::ptr ev) {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_usm::fill");
    cl::Event ev_ocl = dynamic_cast<base_event*>(ev.get())->get();
...
void gpu_usm::zero_buffer() {
    ...
    // [WA]
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_usm::zero_buffer");
    event_impl::ptr ev{ new base_event(_engine->get_context()), false };
    fill(0, ev);
    ev->wait();
}
...
void gpu_usm::copy_from_other(const gpu_usm& other) {
    logger_scope_internal fscope(const_cast<engine_impl*>(_engine), "gpu_usm::copy_from_other");
